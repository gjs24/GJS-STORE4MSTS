    ...
    try {
      ...
      if (results.length > 0) {
        const mapped: BoardTemplate[] = results.map((item: any) => ({
          ...
          allowUserEditTextureName: item.allow_user_edit_texture_name !== undefined ? item.allow_user_edit_texture_name : (item.allowUserEditTextureName !== undefined ? item.allowUserEditTextureName : false),
  ...
  async syncCloudTemplate(template: BoardTemplate): Promise<boolean> {
    try {
      ...
      const payload = {
        ...
        allow_user_edit_texture_name: !!template.allowUserEditTextureName,
