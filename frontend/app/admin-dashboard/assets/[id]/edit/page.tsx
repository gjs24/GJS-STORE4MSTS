...
export default function EditAssetPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      adminGetRequired<EditableAsset>(`/admin/assets/${id}/`)
        .then((data) => {
          ...
          setSelectedBoardTemplateId(
            data.board_template?.id || (data as any).board_template_id || ""
          );
          setBundleBoardTemplateFree(Boolean((data as any).bundle_board_template_free));
          setIsFree(data.is_free);
          setEarlyAccessEnabled(Boolean(data.early_access_enabled));
          setEarlyAccessHasAccess(Boolean(data.early_access_has_access));
          setEarlyAccessHasDiscount(Boolean(data.early_access_has_discount));
          setEarlyAccessDiscountPercent(Number(data.early_access_discount_percent) || 0);
          setEarlyAccessPrice(data.early_access_price && data.early_access_price !== "0.00" ? data.early_access_price : "");
          ...
          setPrebookingStartsAt(data.prebooking_starts_at ? formatDateTimeLocal(data.prebooking_starts_at) : "");
          setPrebookingEndsAt(data.prebooking_ends_at ? formatDateTimeLocal(data.prebooking_ends_at) : "");
  ...
  async function updateAsset(formData: FormData) {
    ...
    formData.set("deal_starts_at", String(formData.get("deal_starts_at") || ""));
    formData.set("deal_ends_at", String(formData.get("deal_ends_at") || ""));
    ...
    formData.set("release_date", String(formData.get("release_date") || ""));
    ...
    <AdminLayout title={`Edit: ${asset.title}`}>
      ...
      <form action={updateAsset} className="grid gap-4 rounded border border-white/10 bg-white/[0.03] p-5 md:grid-cols-2">
        ...
        <label className="block md:col-span-2">
          ...
          <textarea name="media_gallery_urls" rows={4} defaultValue={asset.media_gallery_urls || ""} placeholder={"https://youtube.com/watch?v=demo-video-id\nhttps://res.cloudinary.com/.../video/upload/demo-output.mp4"} className="mt-2 w-full rounded border border-white/10 bg-black/40 px-3 py-3" />
        ...
        <div className="space-y-4 rounded border border-emerald-400/20 bg-emerald-400/5 p-4 md:col-span-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="block">
              ...
              <input name="deal_starts_at" type="datetime-local" defaultValue={formatDateTimeLocal(asset.deal_starts_at)} className="mt-2 w-full rounded border border-emerald-400/30 bg-black/40 px-3 py-3" />
            ...
            <label className="block">
              ...
              <input name="deal_ends_at" type="datetime-local" defaultValue={formatDateTimeLocal(asset.deal_ends_at)} className="mt-2 w-full rounded border border-emerald-400/30 bg-black/40 px-3 py-3" />
            ...
            <div className="space-y-4 pt-1">
              ...
              <div>
                ...
                <div className="max-h-48 overflow-y-auto rounded border border-white/10 bg-black/40 p-2 space-y-1">
                  ...
                  {availableAssets.filter((a) => a.id !== Number(assetId)).length === 0 && (
              ...
              {earlyAccessHasDiscount ? (
                <div className="grid gap-4 md:grid-cols-2 rounded border border-purple-500/20 bg-purple-950/20 p-3">
                  ...
                  <div className="md:col-span-2 text-xs text-purple-300">
                    💡 <strong>Live calculation:</strong> Regular price ₹{asset.price} →{" "}
                    {earlyAccessPrice && Number(earlyAccessPrice) > 0 ? (
                      <span>
                        Eligible buyers pay: <strong>₹{Number(earlyAccessPrice).toFixed(2)}</strong> (saves ₹
                        {Math.max(0, Number(asset.price) - Number(earlyAccessPrice)).toFixed(2)})
                      </span>
                    ) : earlyAccessDiscountPercent > 0 ? (
                      <span>
                        Eligible buyers pay:{" "}
                        <strong>
                          ₹{(Number(asset.price) * (1 - earlyAccessDiscountPercent / 100)).toFixed(2)}
                        </strong>{" "}
                        ({earlyAccessDiscountPercent}% off, saves ₹
                        {(Number(asset.price) * (earlyAccessDiscountPercent / 100)).toFixed(2)})
                      </span>
                    ) : (
                      <span>Set a discount percentage or explicit price above.</span>
                    )}
                  </div>
                </div>
              ) : null}
